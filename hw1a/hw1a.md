...
- For the code generator, I had the model put in comments before main blocks of code explaining the following lines as if I were five years old. I then had it generate code to output the first 100 digits of pi, and it worked.
...
- I explored the middle-ground between rules in my classifier. For example, I classified users into different types of benders based on their personality traits. I then added another rule that stated that if the user demonstrated more than one type of personality trait, the model should output "Avatar." The model did surprisingly well at this.
    * I tested the input "I am free-spirited and a water-bender," and the model output Avatar. Cheaper models took much longer than newer models (6.4 seconds for gpt-4.1 versus 2.6 seconds for gpt-5.6-sol)
...
- For the code generator code, in my prompt I told the model not to use libraries outside of Python's standard libraries, such as numpy. In the input text, I told it to use numpy matrices. Even for the cheaper models like gpt-4.1, it produced a "CANNOT COMPLY error" as it was supposed to.

## Obstacles I encountered
- **Hallucination with cheaper models:** When using cheaper models like `gpt-5-nano`, the model frequently hallucinated — for example, it output a classification even on a run with no input text. This showed me that cheaper models are more likely to give confident-sounding answers with no substance behind them.
...
- **Reasoning effort hardcoded:** I had initially hardcoded reasoning effort to "low." I later dropped this so the program wouldn't fail when using cheaper models, like `gpt-4.1`, that don't support that parameter.
...
- Overall, I believe that cheaper models can do basic tasks just as well as more expensive models. It is important to know and distinguish when it is worth it to use more expensive models based on the difficulty of the task.
...
I spent 5 hours on this homework.
